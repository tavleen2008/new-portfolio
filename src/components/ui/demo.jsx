...
export function BackgroundBoxesDemo() {
  return (
    <div className="h-[calc(100vh-64px)] w-full relative overflow-hidden bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center transition-colors duration-300">
      {/* Animated background boxes */}
      <Boxes />
      
      {/* Radial gradient mask overlay */}
      <div className="absolute inset-0 w-full h-full bg-slate-50 dark:bg-slate-950 z-10 [mask-image:radial-gradient(ellipse_80%_80%_at_50%_50%,transparent_0%,black_100%)] pointer-events-none transition-colors duration-300" />
      ...
      <div className="relative z-20 flex flex-col items-center justify-center text-center px-6">
        <h1 className={cn("text-5xl md:text-7xl lg:text-8xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-700 to-slate-500 dark:from-white dark:via-slate-200 dark:to-slate-400 tracking-tight select-none")}>
          Tavleen kaur
        </h1>
        
        <p className="mt-6 text-base md:text-xl text-purple-600 dark:text-purple-400/90 font-mono tracking-widest uppercase select-none">
          Meet a girl who codes!
        </p>

        {/* Status badge */}
        <div className="mt-10 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-700/60 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md text-xs text-slate-700 dark:text-slate-300 flex items-center gap-2 transition-all duration-300 cursor-default select-none hover:border-slate-300 dark:hover:border-slate-600/80 hover:bg-white/60 dark:hover:bg-slate-900/60">
